...
FileFormatWxListCtrl::FileFormatWxListCtrl(wxWindow* _parent)
{
	Create(_parent, -1, wxDefaultPosition, wxDefaultSize, wxLC_REPORT, wxDefaultValidator, wxT("fileformatlistctrl"));
...
void FileFormatWxListCtrl::setData(DcmFileFormatPtr _fFormat)
{
	try
	{
		this->clear();
...
void FileFormatWxListCtrl::createView()
{
	try
	{
		...
		int count = 0;
		while ( (fFormat->nextObject(stack, 1)).good() )
		{
			dobj = stack.elem(0);
			devr = dobj->ident();
...
void FileFormatWxListCtrl::clear()
{
	try
	{
		this->DeleteAllItems();
