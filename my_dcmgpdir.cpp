 ...
 *  Source File:      $Source: /share/dicom/cvs-depot/dcmtk/dcmdata/apps/dcmgpdir.cc,v $
...
DcmDicomDir* dcmgpdir_main(int argc, char *argv[])
{
                    ...
                    OFBool searchIntoSub = OFFalse)
{
    ...
    if (cond.bad()) {
		...
		tmp << "Error: " << cond.text() << ": reading file: " << fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
                ...
                const OFString& s, OFBool replaceOld = OFTrue)
{
    if (d == NULL) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertString: null DcmItem argument" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    if (elem == NULL) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertString: cannot create DcmElement" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    if (!s.empty()) {
        ...
        if (cond != EC_Normal) {
            ...
            //return OFFalse;
			std::stringstream tmp;
			tmp << "Error: dcmInsertString: cannot put string" << std::endl;
			throw DicomLibException(tmp.str(), __FILE__, __LINE__);
        }
    }
    ...
    if (cond != EC_Normal) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertString: cannot insert element" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
                 ...
                 const long i, OFBool replaceOld = OFTrue)
{
    if (d == NULL) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertInterger: null DcmItem argument" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    if (elem == NULL) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertInterger: cannot create DcmElement" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    switch(tag.getEVR())
    {
    ...
    if (cond != EC_Normal) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertInterger: cannot put integer" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    cond = d->insert(elem, replaceOld);
    if (cond != EC_Normal) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: dcmInsertInterger: cannot insert element" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
...
dcmCopySequence(DcmItem* sink, const DcmTagKey& key, DcmItem* source)
{
    ...
    if (tag.getEVR() != EVR_SQ) {
        ...
        //abort();
		std::stringstream tmp;
		tmp << "internal Error: dcmCopySequence" << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
...
checkExists(DcmItem* d, const DcmTagKey& key, const OFString& fname)
{
    if (!dcmTagExists(d, key)) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: required attribute " << tag.getTagName() << " "
			<< key << " missing in file: " << fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    return OFTrue;
}
...
checkExistsWithStringValue(DcmItem* d, const DcmTagKey& key, const OFString &value, const OFString& fname, OFBool normalize = OFTrue)
{
    ...
    if (!cmp(s, value)) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: attribute " << tag.getTagName()
			<< " " << key << " has other value than expected in file: "
			<< fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
...
checkImage(const OFString& fname, DcmFileFormat *ff)
{
    ...
    */
    ...
    if (m == NULL || m->card() == 0) {
        ...
        //ok = OFFalse;
		std::stringstream tmp;
		tmp << "Error: file not part 10 format (no meta-header): "
			<< fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    if (d == NULL) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: file contrains no data (no dataset): "
			<< fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    */
    ...
    if (mediaSOPClassUID.empty()) {
        ...
        //ok = OFFalse;
		std::stringstream tmp;
		tmp << "Error: MediaStorageSOPClassUID missing in meta-header: "
			<< fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    */
    ...
    if (!found) {
        ...
        //return OFFalse;
		std::stringstream tmp;
		tmp << "Error: invalid sop class (" << sopClassName
			<< ") for " << getProfileName(dicomdirProfile) << " profile: " << fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    ...
    */
    ...
    if (transferSyntax.empty()) {
        ...
        //ok = OFFalse;
		std::stringstream tmp;
		tmp << "Error: TransferSyntaxUID missing in meta-header: "
			<< fname << std::endl;
        throw DicomLibException(tmp.str(), __FILE__, __LINE__);
    }
    switch (dicomdirProfile)
    {
        ...
        case EDDP_CTandMR:
            if (cmp(mediaSOPClassUID, UID_DetachedPatientManagementSOPClass))
            {
                ...
                if (!found) {
                    ...
                    //ok = OFFalse;
					std::stringstream tmp;
					tmp << "Error: " << xferName1 << " or " << xferName2 << " expected: "
						<< fname << std::endl;
					throw DicomLibException(tmp.str(), __FILE__, __LINE__);
                }
            }
        ...
        case EDDP_UltrasoundCCMF:
            ...
            if (!found) {
                ...
                //     << fname << endl;
                //ok = OFFalse;
				throw DicomLibException("Error!", __FILE__, __LINE__);
            }
            break;
#endif
        case EDDP_None:
